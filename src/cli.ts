...
A DenoJS Framework to crypto enable websites with portable identity

Docs: https://gitlab.com/crypteriat/create-ewa/
Bugs: https://gitlab.com/crypteriat/create-ewa/issues
...
async function main() {
  ...
  const command = (hasCommand
    ? String(args.shift())
    : 'dev') as keyof typeof commands

  // prints version
  if (argOptions.v && command != 'upgrade') {
    console.log(`create-ewa.js v${version}`)
    Deno.exit(0)
  }

  // prints .js and deno version
  if (argOptions.version && command != 'upgrade') {
    const { deno, v8, typescript } = Deno.version
    console.log(`create-ewa.js ${version}`)
  ...
  if (
    ['dev', 'start', 'build'].includes(command) &&
    existsFileSync('../import_map.json')
  ) {
    ...
    if (imports['https://deno.land/x/aleph/']) {
      const match = String(imports['https://deno.land/x/aleph/']).match(
                ...
                if (!item.name.startsWith('.')) {
                  items.push(
                    `<li><a href='${path.join(
                      url.pathname,
                      encodeURI(item.name),
                    )}'>${item.name}${item.isDirectory ? '/' : ''}<a></li>`,
                  )
                }
              }
              resp.send(
                createHtml({
                  head: [`<title>create-ewa.js/</title>`],
                  body: `<h1>&nbsp;create-ewa.js/</h1><ul>${Array.from(
                    items,
                  ).join('')}</ul>`,
                }),
                'text/html',
